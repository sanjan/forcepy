...
class TestRetryLogic:
    ...
    def test_retry_on_503(self, mock_sleep, mock_request):
        ...
        response_503 = Mock(status_code=503, content=b'{"message": "Service unavailable"}', headers={'Content-Type': 'application/json'})
...
class TestIterqueryThreading:
    ...
    def test_iterquery_yields_pages(self, mock_http):
        ...
        mock_http.side_effect = [
            {
                'records': [{'Id': f'00{i}'} for i in range(3)],
                'nextRecordsUrl': '/query/page2',
                'done': False
            },
            {
                'records': [{'Id': f'01{i}'} for i in range(2)],
                'nextRecordsUrl': None,
                'done': True
            }
        ]
...
class TestDependentPicklists:
    ...
    def test_get_dependent_picklist_values(self):
        ...
        hardware_values = describe.get_dependent_picklist_values(
            'Sub_Category__c',
        ...
        software_values = describe.get_dependent_picklist_values(
            'Sub_Category__c',
            controlling_value='Software'
        )
    ...
    def test_get_picklist_values_client_method(self, mock_describe):
        ...
        mock_describe_obj = Mock()
        mock_describe_obj.get_dependent_picklist_values.return_value = [
